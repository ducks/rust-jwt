...
fn sign(header: String, payload: String, secret: String, alg: &Alg) -> Vec<u8> {
  ...
  let sig = match *alg {
    Alg::HS256 => {
      hmac(Type::SHA256, secret.as_bytes(), input.as_bytes())
    },
    Alg::HS384 => {
      hmac(Type::SHA384, secret.as_bytes(), input.as_bytes())
    }
    Alg::HS512 => {
      hmac(Type::SHA512, secret.as_bytes(), input.as_bytes())
    }
  };
...
fn json_to_btree(input: Json) -> BTreeMap<String, json::Json> {
  match input {
    Json::Object(obj) => obj.into_iter().map(|(k, v)| (k, match v {
